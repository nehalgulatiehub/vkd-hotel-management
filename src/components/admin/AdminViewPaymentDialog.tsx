...
export function AdminViewPaymentDialog({ open, onOpenChange, bookingId }: AdminViewPaymentDialogProps) {
  ...
  const fetchPaymentData = async () => {
    ...
    try {
      ...
      const [safariRes, hotelRes, vehicleRes, volvoDMRes, volvomDRes, visaRes, cruiseRes] = await Promise.all([
      ...
      const mdTotalAmount = (volvomDRes.data || []).reduce((s, r: any) => s + toAmount(r.total_amount), 0);
      ...
      // Any amount received over the Booking (own hotel) total is adjusted into the
      // other service modules that still have a due amount (safari first, etc.).
