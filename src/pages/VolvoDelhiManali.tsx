...
export default function VolvoDelhiManali() {
  ...
  const fetchVolvoBookings = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("volvo_bookings").select(`*, bookings(id, booking_number, customer_name, email, status, contact_no, booking_type, created_at, notes, agent_id, created_by, total_amount, paid_amount, due_amount, agents(name))`).eq("route", "delhi_manali").order("travel_date", { ascending: false });
    if (error) { toast.error("Failed to load Delhi-Manali volvo bookings"); } else { setVolvoBookings(data || []); }
    setLoading(false);
  };
  ...
  const filteredBookings = volvoBookings.filter(booking => {
    if (filters.searchWithDate) {
      ...
      const bDate = toYMD(booking.travel_date) || toYMD(booking.bookings?.created_at);
  ...
  return (
    <div style={{ padding: 12, fontFamily: "Arial, Helvetica, sans-serif", fontSize: 11 }}>
      <LegacyPanelHeader
        title="Volvo Delhi - Manali Detail"
        className="mb-2"
        right={
          <Button
            variant="link"
        ...
        {loading ? <div style={{ textAlign: "center", padding: 32, color: "#999" }}>Loading...</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: "Arial, Helvetica, sans-serif" }}>
            <thead><tr style={{ backgroundColor: MAROON_LIGHT, color: "#000", fontWeight: "normal" }}>
              <th style={thStyle}>S.No.</th><th style={thStyle}>Type</th><th style={thStyle}>User</th><th style={thStyle}>Customer Details</th><th style={thStyle}>D - M Volvo Detail</th><th style={thStyle}>Date</th><th style={thStyle}>Actions</th>
            </tr></thead>
            <tbody>
              {paginatedItems.length === 0 ? (
                <tr><td colSpan={7} style={{ ...tdStyle, textAlign: "center", padding: 20, color: "#999" }}>No Delhi-Manali volvo bookings found</td></tr>
                    ...
                    {(() => {
                      ...
                      if (isOwner) {
                        actions.push(
                          ...
                          { label: "Add Payment", fn: () => paymentDialog.handleAddPayment(b, { type: "volvo_dm", id: booking.id }) },
                          { label: "View Payment", fn: () => paymentDialog.handleViewPayment(b, { type: "volvo_dm", id: booking.id }) },
                        );
                      }
                      actions.push(
      ...
      <BookingDetailsDialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog} booking={selectedBookingData} serviceType="volvo_dm" serviceData={selectedServiceData} />
