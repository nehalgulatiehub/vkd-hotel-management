...
export default function VolvoDelhiManali() {
  ...
  const fetchVolvoBookings = async () => {
    const { data, error } = await supabase
      .from("volvo_bookings")
      .select("*, bookings(booking_number, customer_name, status, contact_no)")
      .eq("route", "Delhi-Manali")
      .order("travel_date", { ascending: false });
    
    if (error) {
      toast.error("Failed to load Delhi-Manali volvo bookings");
    } else {
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-4 font-semibold">Rate/Seat</th>
                ...
                <tbody>
                  {filteredBookings.map((booking) => (
                    <tr key={booking.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="p-4">₹{booking.rate_per_seat?.toLocaleString() || 0}</td>
              ...
              {filteredBookings.length === 0 && (
                <div className="text-center py-12 text-muted-foreground">
                  No Delhi-Manali volvo bookings found
                </div>
              )}
            </div>
          </CardContent>
