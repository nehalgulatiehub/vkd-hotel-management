...
const fetchHotels = async () => {
  ...
  const filteredBookings = hotelBookings.filter(booking => {
    ...
    if (filters.searchWithDate) {
      ...
      const bookingDate = toYMD(booking.check_in_date) || toYMD(booking.bookings?.created_at);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4">
        <LegacyPanelHeader
          title="Due Amount Another Hotel Detail"
          className="mb-3"
          right={
            <Button
              variant="link"
          ...
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 px-2 py-1.5">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-1">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <select value={filters.hotel} onChange={(e) => setFilters({...filters, hotel: e.target.value})} className={`${legacyFilterInputClass} min-w-[140px]`}>
                  <option value="">-------Select-------</option>
