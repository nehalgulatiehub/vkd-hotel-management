...
export default function ManaliDelhiDue() {
  ...
  const fetchVolvoBookings = async () => {
    ...
    if (error) {
      toast.error("Failed to load Manali-Delhi due amounts");
    } else {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4">
        <LegacyPanelHeader
          title="Manali - Delhi Due Amount"
          className="mb-3"
          right={
            <Button variant="link" className="text-white p-0 h-auto text-sm hover:text-white/80" onClick={() => navigate("/volvo-manali-delhi")}>
              View All Records
            </Button>
          }
        />
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {filteredBookings.length === 0 ? (
                    <tr><td colSpan={7} className="border border-[#c99] px-4 py-8 text-center text-muted-foreground">No Manali-Delhi due amounts found</td></tr>
                  ) : (
