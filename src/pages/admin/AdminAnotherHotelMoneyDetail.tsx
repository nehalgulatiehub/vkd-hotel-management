...
export default function AdminAnotherHotelMoneyDetail() {
  ...
  const filterSection = (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs">
        <div className="flex items-center gap-1"><span>Another Hotel :</span>
          <select value={hotelId} onChange={(e) => setHotelId(e.target.value)} className="border px-1 py-0.5 text-xs min-w-[250px]"><option value="">--Select--</option>{hotels.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}</select>
        </div>
