...
export default function ViewManaliDelhiDue() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data: volvoData } = await supabase.from("volvo_bookings").select("id, booking_id, total_amount, paid_amount, due_amount, notes").eq("route", "manali_delhi");
      const { data: payments } = await supabase.from("payments").select("transporter_id, amount, approval_status").in("payment_type", [...SERVICE_PAYMENT_TYPES.manaliDelhi]).not("transporter_id", "is", null);
