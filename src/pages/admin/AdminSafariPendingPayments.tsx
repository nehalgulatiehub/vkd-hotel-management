...
export default function AdminSafariPendingPayments() {
  ...
  const handleBulkApproval = async (status: "approved" | "rejected") => {
    ...
    try {
      // Get full payment details for syncing service tables
      ...
      // Sync service tables when approved
        ...
        <Card>
          <CardHeader className="py-2 px-4 border-b bg-destructive/10">
            <CardTitle className="text-sm flex items-center justify-between">
              <span className="flex items-center gap-2"><Clock className="h-4 w-4" /> Safari Pending Payments ({totalItems})</span>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">No pending safari payments found.</div>
            ) : (
              <>
