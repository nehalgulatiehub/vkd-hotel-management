...
export default function AdminSafariPendingPayments() {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("payments").select(`id, amount, payment_date, payment_mode, approval_status, booking:bookings(id, booking_number, customer_name)`).eq("payment_type", "safari").eq("approval_status", "pending").order("payment_date", { ascending: false });
  ...
  if (authLoading) return <div className="min-h-screen"><AdminHeader title="Safari Pending Payments" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
      ...
      <main className="p-4 space-y-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">Safari Pending Payments - Total: ₹{totalAmount.toLocaleString()}</CardTitle>
          ...
          <CardContent>
            ...
            {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : filteredPayments.length === 0 ? <div className="text-center py-8 text-muted-foreground">No pending safari payments found.</div> : (
              <>
                <Table>
