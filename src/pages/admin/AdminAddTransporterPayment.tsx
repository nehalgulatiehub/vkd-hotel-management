...
export default function AdminAddTransporterPayment() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("payments").insert({ amount: parseFloat(payment), payment_mode: paymentMode, reference_number: chequeNo || null, notes: paymentDetail || null, payment_type: "vehicle_direct", payment_date: new Date().toISOString().split('T')[0], created_by: user?.id, approval_status: "pending", transporter_id: selectedTransporter || null });
      if (error) throw error;
      toast.success("Transporter payment added successfully"); handleReset();
  ...
  return (
    <div style={{ padding: 12, fontFamily: "Arial, Helvetica, sans-serif", fontSize: 11 }}>
      <div style={{ fontSize: 13, fontWeight: "bold", marginBottom: 8, color: "#333" }}>📋 Add Transporter Payment</div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
        <span onClick={() => navigate("/admin/vehicle-pending")} style={{ color: "#c00", cursor: "pointer", textDecoration: "underline", fontSize: 11 }}>Manage Transporter Payment</span>
      </div>
      <div style={{ border: "1px solid #ccc" }}>
        <div style={{ backgroundColor: "#b44a50", color: "#fff", padding: "4px 10px", fontSize: 11, fontWeight: "bold" }}>Add Transporter Payment</div>
        <form onSubmit={handleSubmit} style={{ padding: 16, backgroundColor: "#fff" }}>
          ...
          <div style={{ marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select value={selectedTransporter} onChange={e => setSelectedTransporter(e.target.value)} style={{ ...filterSelectStyle, border: "1px solid #999", width: 300 }} required>
