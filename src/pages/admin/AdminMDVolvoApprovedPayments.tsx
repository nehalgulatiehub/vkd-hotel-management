...
export default function AdminMDVolvoApprovedPayments() {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("payments").select(`id, amount, payment_date, payment_mode, approved_at, booking:bookings(id, booking_number, customer_name)`).eq("payment_type", "manali_delhi").eq("approval_status", "approved").order("approved_at", { ascending: false });
      ...
      if (bookingIds.length > 0) {
        ...
        hotelBookings?.forEach((hb: any) => {
          hotelBookingsMap[hb.booking_id] = { hotel_name: hb.hotel?.name || hb.own_hotel?.name || null, room_name: roomsMap[hb.room_type] || hb.room_type || null };
        });
      }
  ...
  if (authLoading) return <div className="min-h-screen"><AdminHeader title="M-D Volvo Approved Payments" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
  ...
  return (
    <div className="min-h-screen">
      <AdminHeader title="M-D Volvo Approved Payments" />
      <main className="p-4 space-y-4">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-600" /> Manali-Delhi Volvo Approved - Total: ₹{totalAmount.toLocaleString()}</CardTitle></CardHeader>
          <CardContent>
            ...
            {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : filteredPayments.length === 0 ? <div className="text-center py-8 text-muted-foreground">No approved M-D Volvo payments found.</div> : (
              <>
                <Table>
