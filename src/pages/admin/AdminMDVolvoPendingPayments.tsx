...
export default function AdminMDVolvoPendingPayments() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("payments")
        .select(`id, amount, payment_date, payment_mode, approval_status, booking:bookings(id, booking_number, customer_name)`)
  ...
  if (authLoading) return <div className="min-h-screen"><AdminHeader title="M-D Volvo Pending Payments" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
  ...
  return (
    <div className="min-h-screen">
      <AdminHeader title="M-D Volvo Pending Payments" />
      <main className="p-4 space-y-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">Manali-Delhi Volvo Pending - Total: ₹{totalAmount.toLocaleString()}</CardTitle>
          ...
          <CardContent>
            ...
            {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : filteredPayments.length === 0 ? <div className="text-center py-8 text-muted-foreground">No pending M-D Volvo payments found.</div> : (
              <>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Booking #</TableHead><TableHead>Customer</TableHead><TableHead>Date</TableHead><TableHead>Mode</TableHead><TableHead className="text-right">Amount</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
