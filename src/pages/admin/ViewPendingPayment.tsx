...
export default function ViewPendingPayment() {
  ...
  const filteredPayments = payments.filter(payment => {
    ...
    const matchesUser = filterUser === "all" || 
      payment.created_by_profile?.username === filterUser ||
      payment.created_by_profile?.first_name === filterUser;
    const matchesAgent = filterAgent === "all" || 
      payment.booking?.agent?.name === filterAgent;
    const matchesHotel = filterHotel === "all" || 
      payment.hotel_info?.hotel_name === filterHotel;
  ...
  const fetchPendingPayments = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("payments")
        .select(`
          id, amount, payment_mode, payment_date, reference_number, approval_status, created_at, created_by,
          booking:bookings(id, booking_number, check_in_date, check_out_date, customer_name, contact_no, total_amount, adults, children, address, agent:agents(name, company_name))
        `)
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedPayments(new Set(paginatedItems.map(p => p.id)));
    } else {
  ...
  if (authLoading) {
    return <div className="min-h-screen"><AdminHeader title="View Pending Payment" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
  }

  if (!isAdmin()) {
    return <div className="min-h-screen"><AdminHeader title="Access Denied" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Admin access required.</CardContent></Card></main></div>;
  }

  return (
    <div className="min-h-screen">
      <AdminHeader title="View Pending Payment" />
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <>
                    ...
                    <TableHeader>
                      <TableRow className="bg-muted/50">
                        ...
                        <TableHead className="w-10">
                          <Checkbox 
                            checked={paginatedItems.length > 0 && selectedPayments.size === paginatedItems.length}
                    ...
                    <TableBody>
                      {paginatedItems.map((payment, index) => (
                        <TableRow key={payment.id} className="hover:bg-muted/30">
                          ...
                          <TableCell>
                            <Badge variant="secondary" className="text-orange-600 border-orange-300">Pending</Badge>
                          </TableCell>
                          <TableCell>
                            <Checkbox 
                ...
                {/* Footer with pagination and approval button */}
                <div className="flex items-center justify-between p-4 border-t">
                  <div className="text-sm font-medium">
                    Total Payment: Rs. {totalPaymentAmount.toLocaleString()}/-
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        Approved ({selectedPayments.size})
                      </Button>
                      <Button 
                        variant="destructive" 
                        size="sm" 
