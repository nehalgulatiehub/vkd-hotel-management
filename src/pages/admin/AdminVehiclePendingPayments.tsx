...
export default function AdminVehiclePendingPayments() {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("payments").select(`id, amount, payment_date, payment_mode, approval_status, booking:bookings(id, booking_number, customer_name)`).eq("payment_type", "vehicle").eq("approval_status", "pending").order("payment_date", { ascending: false });
  ...
  const handleSelectPayment = (paymentId: string, checked: boolean) => { setSelectedPayments(prev => { const newSet = new Set(prev); checked ? newSet.add(paymentId) : newSet.delete(paymentId); return newSet; }); };
  ...
  const handleSingleApproval = async (paymentId: string, status: "approved" | "rejected") => {
    try {
      const { error } = await supabase.from("payments").update({ approval_status: status, approved_by: user?.id, approved_at: new Date().toISOString() }).eq("id", paymentId);
      if (error) throw error;
  ...
  if (authLoading) return <div className="min-h-screen"><AdminHeader title="Vehicle Pending Payments" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
  ...
  return (
    <div className="min-h-screen">
      <AdminHeader title="Vehicle Pending Payments" />
      <main className="p-4 space-y-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base">Vehicle Pending Payments - Total: ₹{totalAmount.toLocaleString()}</CardTitle>
          ...
          <CardContent>
            ...
            {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : filteredPayments.length === 0 ? <div className="text-center py-8 text-muted-foreground">No pending vehicle payments found.</div> : (
              <>
                <Table>
                  <TableHeader><TableRow><TableHead className="w-10"><Checkbox checked={paginatedItems.length > 0 && selectedPayments.size === paginatedItems.length} onCheckedChange={handleSelectAll} /></TableHead><TableHead>Booking #</TableHead><TableHead>Customer</TableHead><TableHead>Date</TableHead><TableHead>Mode</TableHead><TableHead className="text-right">Amount</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
                  <TableBody>
