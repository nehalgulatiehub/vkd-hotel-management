...
export default function ViewVehicleDue() {
  ...
  const [filters, setFilters] = useState({
    ...
    type: "", agentName: "", transporter: "", user: "", customer: "", vehicle: "",
  ...
  const fetchTransporters = async () => {
    const { data } = await supabase.from("transporters").select("*").order("name");
  ...
  const filteredVehicles = vehicles.filter(vehicle => {
    const matchesType = !filters.type || vehicle.booking?.booking_type === filters.type;
    const matchesAgent = !filters.agentName || vehicle.booking?.agents?.name?.toLowerCase().includes(filters.agentName.toLowerCase());
    const matchesTransporter = !filters.transporter || vehicle.transporter?.name?.toLowerCase().includes(filters.transporter.toLowerCase());
    const matchesCustomer = !filters.customer || vehicle.booking?.customer_name?.toLowerCase().includes(filters.customer.toLowerCase());
    const matchesVehicle = !filters.vehicle || vehicle.vehicle_number?.toLowerCase().includes(filters.vehicle.toLowerCase());
    
    let matchesDate = true;
    if (filters.searchWithDate && filters.fromYear && filters.fromMonth && filters.fromDay && vehicle.pickup_date) {
      ...
      matchesDate = pickupDate >= fromDate;
      ...
      if (filters.toYear && filters.toMonth && filters.toDay) {
        ...
        matchesDate = matchesDate && pickupDate <= toDate;
      }
    }
    
    return matchesType && matchesAgent && matchesTransporter && matchesCustomer && matchesVehicle && matchesDate;
  });

  const totalDue = filteredVehicles.reduce((sum, v) => sum + (v.due_amount || 0), 0);
  const totalAmount = filteredVehicles.reduce((sum, v) => sum + (v.total_amount || 0), 0);
  const totalPaid = filteredVehicles.reduce((sum, v) => sum + (v.paid_amount || 0), 0);

  const handleViewDetails = (vehicle: any) => {
    setSelectedBooking(vehicle);
  ...
  const handleViewPayment = async (vehicle: any) => {
    setSelectedBooking(vehicle);
    setBookingPayments([]);
    setShowViewPaymentDialog(true);
    const bookingId = vehicle.booking?.id;
    if (bookingId) {
      await fetchBookingPayments(bookingId);
    }
  };

  const handleAddPayment = (vehicle: any) => {
    setSelectedBooking(vehicle);
    setPaymentAmount("");
    setPaymentMode("");
    setPaymentReference("");
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen">
        <AdminHeader title="Vehicle Due Amount" />
        <main className="p-4">
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4">
        ...
        <div className="flex justify-between items-center px-4 py-2 mb-3" style={{ backgroundColor: "#1e6e99" }}>
          ...
          <Button variant="link" className="text-white p-0 h-auto text-sm hover:text-white/80" onClick={() => navigate("/admin/vehicle-details")}>
            View All Records
          </Button>
        </div>
        ...
        <div className="mb-3 border border-border bg-muted/50">
          ...
          {/* Row 3: User, Customer, Vehicle, Search */}
        ...
        <Card>
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">No vehicle due amounts found</div>
            ) : (
              <>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr style={{ backgroundColor: "#D4A59A" }}>
                        ...
                        <th className="border border-[#c99] px-3 py-2 text-left text-xs font-semibold">Route</th>
                    ...
                    <tbody>
                      {filteredVehicles.map((vehicle, index) => (
                        <tr key={vehicle.id} style={{ backgroundColor: "#F5E6E0" }}>
                          ...
                          <td className="border border-[#c99] px-3 py-2 text-xs align-top">
                            <div className="capitalize">{vehicle.booking?.booking_type || "Direct"}</div>
                            <div className="text-muted-foreground">{vehicle.booking?.agents?.name || ""}</div>
                          </td>
                          <td className="border border-[#c99] px-3 py-2 text-xs align-top">
                            <div className="font-medium">{vehicle.booking?.customer_name || "-"}</div>
                            {vehicle.booking?.address && <div className="text-muted-foreground">{vehicle.booking.address}</div>}
                            <div className="text-muted-foreground">Contact No.: {vehicle.booking?.contact_no || ""}</div>
                          </td>
                          <td className="border border-[#c99] px-3 py-2 text-xs align-top">
                            <div><strong>Vehicle :</strong> {vehicle.vehicle_type || "-"} ({vehicle.vehicle_number || "-"})</div>
                            <div><strong>Transporter :</strong> {vehicle.transporter?.name || "-"}</div>
                            <div><strong>Booking Price :</strong> Rs. {vehicle.total_amount?.toLocaleString("en-IN") || 0}/-</div>
                            <div><strong>Total Received :</strong> Rs. {vehicle.paid_amount?.toLocaleString("en-IN") || 0}/-</div>
                            <div className="text-destructive"><strong>Due Payment :</strong> Rs. {vehicle.due_amount?.toLocaleString("en-IN") || 0}/-</div>
                          </td>
                          <td className="border border-[#c99] px-3 py-2 text-xs align-top">
                            <div>{vehicle.from_location} → {vehicle.to_location}</div>
                          </td>
                          <td className="border border-[#c99] px-3 py-2 text-xs align-top">
                            <div><strong>Booking Date :</strong> {vehicle.booking?.created_at ? new Date(vehicle.booking.created_at).toLocaleDateString("en-GB") : "-"}</div>
                            <div><strong>Pickup Date :</strong> {vehicle.pickup_date ? new Date(vehicle.pickup_date).toLocaleDateString("en-GB") : "-"}</div>
                          </td>
                          <td className="border border-[#c99] px-3 py-2 align-top">
                            <div className="flex flex-col gap-0.5">
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => handleViewDetails(vehicle)}>View Details</Button>
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => vehicle.booking && navigate(`/bookings/${vehicle.booking.id}`)}>Print Booking</Button>
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => vehicle.booking && navigate(`/admin/bookings?edit=${vehicle.booking.id}`)}>Edit Booking</Button>
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => handleAddPayment(vehicle)}>Add Payment</Button>
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => handleViewPayment(vehicle)}>View Payment</Button>
                              <Button size="sm" variant="link" className="h-auto p-0 text-[11px] text-primary justify-start" onClick={() => vehicle.booking && navigate(`/refunds?booking_id=${vehicle.booking.id}`)}>Refund Payment</Button>
                            </div>
                          </td>
                        </tr>
                      ))}
