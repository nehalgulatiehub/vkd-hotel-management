...
export default function ViewDelhiManaliDue() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Build summaries - use transporters that have payments
      const { data: transportersList } = await supabase
        .from("transporters")
        .select("id, name");
      ...
      for (const [tId, info] of Object.entries(transporterMap)) {
        const name = tMap.get(tId) || "Unknown";
        results.push({
          transporter_id: tId,
      ...
      // If no transporter payments exist, show aggregate
      if (results.length === 0 && totalVolvoAmount > 0) {
        ...
        results.push({
          ...
          transporter_name: "All DM Transporters",
          total_transport_amount: totalVolvoAmount,
      ...
      console.error("Error fetching DM due data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen">
        <AdminHeader title="Due DM Transporter Payment" />
        <main className="p-4">
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4">
        ...
        <div className="flex justify-between items-center px-4 py-2 mb-3" style={{ backgroundColor: "#1e6e99" }}>
          <span className="text-white font-semibold text-sm">View DM Transporter Payment</span>
          <Button variant="link" className="text-white p-0 h-auto text-sm hover:text-white/80" onClick={handleViewAllRecords}>
            View All Records
          </Button>
        </div>
                        ...
                        <tr key={summary.transporter_id} style={{ backgroundColor: "#F5E6E0" }}>
                          ...
                          <td className="border border-[#c99] px-3 py-2 text-xs">
                            <Button
                              ...
                              onClick={() => navigate(`/admin/dm-transporter-money?transporter=${summary.transporter_id}`)}
                            >
                              View
                            </Button>
                          </td>
