...
export default function ViewDelhiManaliDue() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (results.length === 0 && totalVolvoAmount > 0) {
        ...
        results.push({ transporter_id: "aggregate", transporter_name: "All DM Transporters", total_transport_amount: totalVolvoAmount, total_paid: totalPaid, outstanding: totalPaid - totalVolvoAmount });
      }
      setSummaries(results);
    } catch (error) { console.error("Error fetching DM due data:", error); }
  ...
  return (
    <AdminPageShell
      title="Due DM Transporter Payment"
          ...
          <tbody>
            {pagination.paginatedItems.length === 0 ? <ThemedEmptyRow colSpan={6} message="No records found" /> : pagination.paginatedItems.map((summary, index) => (
              <ThemedTR key={summary.transporter_id} index={index}>
                ...
                <ThemedTD>
                  <span onClick={() => navigate(`/admin/dm-transporter-money?transporter=${summary.transporter_id}`)} style={{ color: "#c00", cursor: "pointer", fontSize: 10 }} onMouseEnter={(e) => (e.currentTarget.style.textDecoration = "underline")} onMouseLeave={(e) => (e.currentTarget.style.textDecoration = "none")}>View</span>
                </ThemedTD>
              </ThemedTR>
            ))}
          </tbody>
