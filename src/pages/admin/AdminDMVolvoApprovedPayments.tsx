...
export default function AdminDMVolvoApprovedPayments() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("payments")
        .select(`id, amount, payment_date, payment_mode, approved_at, booking:bookings(id, booking_number, customer_name)`)
  ...
  if (authLoading) {
    return <div className="min-h-screen"><AdminHeader title="D-M Volvo Approved Payments" /><main className="p-4"><Card><CardContent className="py-8 text-center text-muted-foreground">Loading...</CardContent></Card></main></div>;
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="p-4 space-y-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-600" /> Delhi-Manali Volvo Approved - Total: ₹{totalAmount.toLocaleString()}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">No approved D-M Volvo payments found.</div>
            ) : (
              <>
                <Table>
                  <TableHeader>
