...
export default function HotelPayments() {
  ...
  const filteredPayments = payments.filter(payment => {
    ...
    const matchesHotel = !hotelFilter || 
      payment.hotel_booking?.another_hotels?.name?.toLowerCase().includes(hotelFilter.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header title="Another Hotel Payment" />
      <main className="p-4">
        {/* Header Bar */}
        <div className="bg-[#1e6e99] text-white px-4 py-2 flex justify-between items-center">
          <span className="font-semibold">Another Hotel Payment</span>
          <Link to="/payments/hotel-due" className="text-white hover:underline text-sm">View All Records</Link>
        </div>
          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="bg-[#D4A59A] text-black">
                ...
                <th className="border border-[#c99] px-3 py-2 text-left font-semibold">Another Hotel</th>
              ...
              {filteredPayments.map((payment, index) => (
                <tr key={payment.id} className={index % 2 === 0 ? "bg-[#F5E6E0]" : "bg-[#FDE1E1]"}>
                  ...
                  <td className="border border-[#c99] px-3 py-2 align-top">
                    {payment.hotel_booking?.another_hotels?.name || "-"}
                  </td>
                  <td className="border border-[#c99] px-3 py-2 align-top">
