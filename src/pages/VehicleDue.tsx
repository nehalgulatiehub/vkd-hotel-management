...
export default function VehicleDue() {
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("payments").insert({ booking_id: bookingId, amount: parseFloat(paymentAmount), payment_mode: paymentMode, reference_number: paymentReference, payment_date: new Date().toISOString().split('T')[0] });
      if (error) throw error;
      toast.success("Payment added successfully");
      setShowAddPaymentDialog(false);
      fetchVehicleBookings();
    } catch (error) { console.error("Payment error:", error); toast.error("Failed to add payment"); }
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {filteredBookings.length === 0 ? (
                    <tr><td colSpan={7} className="border border-[#c99] px-4 py-8 text-center text-muted-foreground">No vehicle due amounts found</td></tr>
                  ) : (
