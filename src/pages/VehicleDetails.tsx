...
export default function VehicleDetails() {
  ...
  const fetchVehicleBookings = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("vehicle_bookings").select(`*, bookings(id, booking_number, customer_name, email, status, contact_no, booking_type, created_at, notes, agent_id, created_by, total_amount, paid_amount, due_amount, agents(name)), transporters(name)`).order("pickup_date", { ascending: false });
  ...
  const filteredBookings = vehicleBookings.filter(booking => {
    if (filters.searchWithDate) {
      ...
      const bDate = toYMD(booking.pickup_date) || toYMD(booking.bookings?.created_at);
    ...
    if (filters.vehicle && !booking.vehicle_type?.toLowerCase().includes(filters.vehicle.toLowerCase()) && !booking.vehicle_number?.toLowerCase().includes(filters.vehicle.toLowerCase())) return false;
      ...
      <LegacyPanelHeader
        ...
        right={
          <Button
            ...
            onClick={() => { setFilters(getDefaultFilters()); fetchVehicleBookings(); }}
          >
            View All Records
          </Button>
        }
        ...
        {loading ? <div style={{ textAlign: "center", padding: 32, color: "#999" }}>Loading...</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: "Arial, Helvetica, sans-serif" }}>
            <thead><tr style={{ backgroundColor: MAROON_LIGHT, color: "#000", fontWeight: "normal" }}>
              <th style={thStyle}>S.No.</th><th style={thStyle}>Type</th><th style={thStyle}>User</th><th style={thStyle}>Customer Details</th><th style={thStyle}>Vehicle Detail</th><th style={thStyle}>Date</th><th style={thStyle}>Actions</th>
            </tr></thead>
            <tbody>
              {paginatedItems.length === 0 ? (
                <tr><td colSpan={7} style={{ ...tdStyle, textAlign: "center", padding: 20, color: "#999" }}>No vehicle bookings found</td></tr>
                ...
                <tr key={booking.id}>
                  ...
                  <td style={tdStyle}>
                    <div><strong>Journey Date</strong></div><div>:{booking.pickup_date ? format(new Date(booking.pickup_date), "dd/MM/yyyy") : "-"}</div>
