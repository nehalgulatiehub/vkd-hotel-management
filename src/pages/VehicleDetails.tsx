...
export default function VehicleDetails() {
  ...
  const filteredBookings = vehicleBookings.filter(booking => {
    if (filters.searchWithDate) {
      const journeyDate = new Date(booking.pickup_date);
    ...
    if (filters.vehicle) {
      if (!booking.vehicle_type?.toLowerCase().includes(filters.vehicle.toLowerCase()) && 
          !booking.vehicle_number?.toLowerCase().includes(filters.vehicle.toLowerCase())) return false;
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-2">
        <div className="flex justify-between items-center mb-2 px-3 py-2" style={{ backgroundColor: "#1e6e99" }}>
          <h2 className="text-white font-semibold text-sm">View Vehicle Details</h2>
          <button className="text-white text-sm hover:underline">View All Records</button>
        </div>

        <DetailPageFilters
          options={{ showType: true, showAgent: true, showUser: true, showCustomer: true, showReference: true, showTransporter: true, showVehicle: true }}
          filters={filters}
        ...
        <div className="mt-2 overflow-x-auto">
            ...
            <table className="w-full text-[11px] border-collapse">
              <thead>
                <tr style={{ backgroundColor: "#D4A59A" }}>
                  ...
                  <th className="border border-[#c99] px-2 py-1.5 text-left font-semibold">Vehicle Detail</th>
                ...
                {paginatedItems.map((booking, idx) => (
                  <tr key={booking.id} style={{ backgroundColor: "#F5E6E0" }}>
                    ...
                    <td className="border border-[#c99] px-2 py-2 align-top">
                      ...
                      <div>:{booking.pickup_date ? format(new Date(booking.pickup_date), "dd/MM/yyyy") : "-"}</div>
          ...
          {!loading && filteredBookings.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">No vehicle bookings found</div>
          )}
        </div>
